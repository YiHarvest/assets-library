...
function promptFor(mediaType: MediaType, durationSeconds: number | null) {
  ...
  return [
    ...
    "分析结果用于短视频文案逐段匹配画面。保留可复用的视觉事实与关键技术名称，人物标签保留可见的人数与大致年龄段。不针对某篇文案编造用途，不为凑数补标签。",
    "视频 topics 最多 5 项：概括具体主题，并补充画面有依据、剪辑时可用于表达的活动、功能或概念。优先使用普通文案中的常用中文，避免全部写成专业零件名或界面细节；例如可辨认的神经网络处理器可表达人工智能算力，订单购物界面可表达电商购物。表达主题只写在 topics；description、timeline、keyMoments 保持可见事实，不把表达主题当成已发生的事件，不推断收益、成败、亲属关系或未知品牌。",
