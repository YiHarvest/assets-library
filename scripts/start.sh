#!/usr/bin/env bash
# 启动 assets-library 的所有服务：分镜服务 + Next.js Web + worker
...
required_env=(PORT API_INTERNAL_ORIGIN WEB_LISTEN_HOST)
...
echo "  Web 与已启用的辅助服务均已就绪。"
echo "  模式:    $APP_MODE  (改 .env 的 APP_MODE=dev 切开发模式)"
echo "  日志:    $PID_DIR/{scene,app}.log"
