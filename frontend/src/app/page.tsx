...
import { pendingFiles } from "@/lib/pending-assets";
...
export default async function OverviewPage({
  ...
  const page = view === "pending"
    ? await serverApi<PendingTaskListResponse>(
        `/tasks?view=pending&user_id=${encodeURIComponent(userId)}&limit=8${
        ...
        }`,
        {
          ...
          telemetryMetadata: {
            view,
            user_scope: userId ? "personal" : "public",
          },
        },
      ).then((result) => ({
        total_files: pendingFiles(result.tasks, userId).length,
        image_files: 0,
        video_files: 0,
        files: pendingFiles(result.tasks, userId),
        next_cursor: result.next_cursor,
        has_more: result.has_more,
      }))
    : tagQuery
      ? await serverApi<AssetListResponse>("/assets/search", {
        method: "POST",
        body: JSON.stringify({
          user_id: userId || null,
          tags: { any: [tagQuery] },
      ...
      : await serverApi<AssetListResponse>("/assets/list", {
        method: "POST",
        body: JSON.stringify({
          user_id: userId || null,
          phases: ["published"],
  ...
  return (
    <main className="mx-auto max-w-7xl px-5 py-7 sm:py-9">
      ...
      <div className="mb-7 flex flex-col gap-3 rounded-[1.5rem] border border-black/[0.06] bg-white/70 p-3 shadow-sm backdrop-blur-xl dark:border-white/[0.10] dark:bg-white/[0.06] sm:flex-row sm:items-center">
        ...
        {view === "published" ? (
          <form action={prefixedHref("/")} method="get" className="flex flex-1 items-center gap-2">
