...
describe("short video combinations", () => {
  it.each([1000, 1500, 2000])("uses %s ms for both member eligibility and the minimum combined duration", minimum => {
    const half = minimum / 2;
    const found = search({ a: half, b: half, regular: minimum });
    expect(closestShortVideos(found.items, found.shortVideoDurations!, minimum, minimum)?.map(part => part.id)).toEqual(["a", "b"]);
    const pools = addShortVideoGroups([{ start_time: 0, end_time: minimum / 1000 }], [[]], [found], minimum);
    expect(pools[0].length).toBeGreaterThan(0);
    for (const material of pools[0]) expect(material.parts?.map(part => part.id).sort()).toEqual(["a", "b"]);
    const insufficient = search({ a: half - 1, b: half });
    expect(addShortVideoGroups([{ start_time: 0, end_time: 2 }], [[]], [insufficient], minimum)).toEqual([[]]);
  });
  ...
  it("cannot pad a short video with another public/private copy of itself", () => {
    ...
    expect(addShortVideoGroups(timeline.slice(0, 1), [[]], [found], 2000)).toEqual([[]]);
    const parts = found.items.map(item => ({ ...item, mediaIdentity: "same-file" }));
    expect(closestShortVideos(parts, found.shortVideoDurations!, 2500, 2000)).toBeUndefined();
  });
  ...
  it("does not extend a group with a copy already assigned to another segment", () => {
    ...
    fitShortVideoGroups([{ start_time: 0, end_time: 2 }, { start_time: 2, end_time: 6 }], assignment, [search({}), found], 2000);
  ...
  it("finds exactly 0.9 + 1.1 seconds instead of greedily taking 1.9 + 1.8 for a short target", () => {
    ...
    expect(closestShortVideos(found.items, found.shortVideoDurations!, 1480, 2000)?.map(part => part.id)).toEqual(["c", "d"]);
  });
  it("keeps a combination shorter than the target if it still reaches two seconds", () => {
    const found = search({ a: 1200, b: 1300 });
    expect(closestShortVideos(found.items, found.shortVideoDurations!, 5000, 2000)?.map(part => part.id)).toEqual(["a", "b"]);
  });
  it("rejects insufficient, unknown and repeated footage", () => {
    const found = search({ a: 999, b: 1000, long: 2000, zero: 0 });
    expect(closestShortVideos(found.items, found.shortVideoDurations!, 1480, 2000)).toBeUndefined();
    expect(closestShortVideos([item("a"), item("a"), item("unknown")], { a: 1600 }, 1480, 2000)).toBeUndefined();
  });
  it("distributes disjoint combinations through the timeline without using a source twice", () => {
    ...
    const pools = addShortVideoGroups(timeline, timeline.map(() => []), searches, 2000);
  ...
  it("reserves scarce later matches and requires every component to match the assigned text", () => {
    ...
    const selected = balancedAssetAssignment(timeline, addShortVideoGroups(timeline, timeline.map(() => []), searches, 2000));
    ...
    expect(addShortVideoGroups(timeline.slice(0, 2), [[], []], unrelated, 2000)).toEqual([[], []]);
  });
  it("allows a better short combination to compete with a weaker regular video", () => {
    ...
    const selected = balancedAssetAssignment(timeline.slice(0, 1), addShortVideoGroups(timeline.slice(0, 1), regular, [short], 2000));
  ...
  it("does not reserve invisible components for a subsecond target", () => {
    ...
    expect(addShortVideoGroups([{ start_time: 0, end_time: 0.44 }], [[]], [short], 2000)).toEqual([[]]);
    expect(addShortVideoGroups([{ start_time: 0, end_time: 0.44 }], [[]], [short], 2000, false)[0]).not.toHaveLength(0);
  });
  it("accounts for output frame rounding before reserving another source", () => {
    ...
    expect(addShortVideoGroups([segment], [[]], [short], 2000)).toEqual([[]]);
    expect(closestShortVideos(short.items, short.shortVideoDurations!, 1480, 2000)).toBeUndefined();
  });
  it("requires matching evidence inside each member's actual played prefix", () => {
    ...
    expect(addShortVideoGroups([{ start_time: 0, end_time: 1.48 }], [[]], [short], 2000)).toEqual([[]]);
    short.playbackEvidence.b[0].startMs = 200;
    const pools = addShortVideoGroups([{ start_time: 0, end_time: 1.48 }], [[]], [short], 2000);
  ...
  it("uses spare clips to approach the actual assigned slot instead of stopping at the minimum duration", () => {
    ...
    const assignment = balancedAssetAssignment(segments, addShortVideoGroups(segments, [[], []], searches, 2000));
    fitShortVideoGroups(segments, assignment, searches, 2000);
