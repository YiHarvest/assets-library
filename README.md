...
- 画廊和列表支持本页多选、全选及批量删除，等待任务完成并分别报告成功和失败。单项、批量删除均保留原页码和筛选；删除后该页不存在时返回最后一页。
...
项目归属使用可选 UUID 字符串 `project_id`：上传清单和 `segment-match` 请求放在顶层，
`assets/query` 放在 `filter.project_id`，用户媒体列表放在 URL 查询参数。MCP 对应上传、查询
和媒体列表工具也支持此字段。上传整批图片、分镜及公私副本继承同一项目；素材列表和详情
返回 `project_id`，旧素材为 `null`。省略或传 `null` 保持原有范围；指定项目时与用户范围、
勾选素材及其他过滤条件取交集，不包含未归属素材。

WebUI 支持上传时填写项目、列表按项目筛选及查看归属；翻页、切换视图、返回列表会保留筛选。
此功能不需要新增 `.env` 配置，也不需要重建 ES 索引。升级时先执行数据库迁移
`0010_asset_projects`（`scripts/start.sh` 会自动执行），再启动新代码。
...
pnpm lint
pnpm test:unit
node tests/browser/asset-deletion.mjs # 真实 WebUI + 内存 API，验证批量删除及分页，不连接数据库
...
2–8 字短搜索词在各 BM25 字段使用连续短语匹配，空格分隔的词分别匹配，避免“夕阳”仅因“阳”命中“阳台”。
素材库短词搜索中，没有完整词法证据的候选还需整片相似度达到本次最佳整片分数减 0.1（且不低于语义门槛），保留语义近义结果并限制局部泛化。此附加准入不用于分段时间轴匹配；无需重建索引。
...
明确无可见内容的黑白屏、纯噪声和测试卡在分析阶段拒绝，存量这类解析也不进入索引或召回。
BM25 分为动作/事件/物体/主题和人物/场景两个方向，均须通过语义门槛。RRF 权重为语义 0.5、
两个词法方向各 0.25，词法总票重不随字段数量增长；字段命中对分配质量最多增加 0.01。
用户勾选素材时，额外对当前有效 `topic` 标签的独立向量进行主题召回，复用短句和语境向量，不增加模型请求。
主题分数使用短句 70% + 语境 30%，残缺短句只用语境；至少达到 `max(SEARCH_SEMANTIC_THRESHOLD, 0.55)`，
且实际播放窗口内的视觉事实仍须达到语义门槛，才能补充直接匹配。主题文档本身永远不是播放证据。
有达标主题候选时，语义的 0.5 票重由画面语义与主题各占 0.25；主题分配质量只计阈值以上收益的 1/4，保留直接匹配优势。
普通素材库搜索不使用主题向量。旧索引无主题文档时沿用原有召回；公共库“夕阳”等短词的过滤规则继续生效。
...
有效 `topic` 标签额外写入 `evidenceKind=theme` 的独立向量文档，与视觉事实分开；索引重建同时补齐该文档。
新 VLM 提示词说明用途是短视频文案逐段匹配画面，要求描述和摘要保留可核验的视觉事实、功能、
技术名称及人数年龄，不针对文案编造用途；视频保留最早提供的关键帧及其准确时间，禁止把后续事实提前。
视频 `topics` 复用现有字段，最多 5 项，包含具体主题及画面支持的表达概念，例如人工智能算力、电商购物；
表达概念不写入时间轴事实，不推断收益、成败、亲属关系或未知品牌，无需修改业务接口。
这些时间来自 VLM 对抽样帧的观察，仍可能有识别误差。存量正确解析无需重跑 VLM，部署后执行下方
`search:reindex` 即可补齐分块及标签文档；重建不会修复已有错误描述，也不会覆盖人工编辑内容。
尚未更新生产代码时，先使用下方 `search:rebuild:staging` 构建独立索引并回放验证，完成代码升级与增量核对后再切换。
重建只使用现有解析和生效标签；要让缺少表达主题的旧素材获得新主题，须先按新提示词重新分析这些素材，再重建索引。
...
- `SEARCH_RRF_K`：RRF 常数，默认 60；语义合计 0.5、事件/物体/主题 0.25、人物/场景 0.25。有达标主题候选时，语义票重在画面与主题之间平分。
