ALTER TABLE `private_assets` ADD `review_status` enum('pending_review','published','deleted') DEFAULT 'pending_review' NOT NULL;--> statement-breakpoint
UPDATE `private_assets` private_asset
LEFT JOIN `public_assets` public_asset ON public_asset.`id` = private_asset.`public_asset_id`
SET private_asset.`review_status` = CASE
  WHEN public_asset.`review_status` = 'published' THEN 'published'
  ELSE 'pending_review'
END;--> statement-breakpoint
CREATE INDEX `private_assets_user_review_created_idx` ON `private_assets` (`user_id`,`review_status`,`created_at`);--> statement-breakpoint
